    ...
    private void FixedUpdate()
    {
        if ((!IN_GAME_MAIN_CAMERA.isPausing || (IN_GAME_MAIN_CAMERA.gametype != GAMETYPE.SINGLE)) && ((IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.SINGLE) || base.photonView.isMine))
        {
            ...
            else if (this.state == "chase")
            {
                ...
                Vector3 vector2 = (Vector3) (base.transform.forward * this.speed);
                Vector3 velocity = base.rigidbody.velocity;
                Vector3 force = vector2 - velocity;
                force.y = 0f;
                base.rigidbody.AddForce(force, ForceMode.VelocityChange);
                float current = 0f;
                Vector3 vector5 = this.myHero.transform.position - base.transform.position;
                current = -Mathf.Atan2(vector5.z, vector5.x) * 57.29578f;
            ...
            else
            {
                ...
                else if (this.state == "attack")
                {
                    ...
                    if (((this.attackAnimation == "jumpCombo_1") && (base.animation["attack_" + this.attackAnimation].normalizedTime >= 0.65f)) && (!this.startJump && (this.myHero != null)))
                    {
                        ...
                        Vector3 vector2 = (Vector3) ((this.myHero.transform.position + (this.myHero.rigidbody.velocity * num13)) + ((((Vector3.up * 0.5f) * num7) * num13) * num13));
                        float num14 = vector2.y;
                        if ((num12 < 0f) || ((num14 - num9) < 0f))
                        {
                            this.idle(0f);
                            num13 = 0.5f;
                            vector2 = base.transform.position + ((Vector3) ((num9 + 5f) * Vector3.up));
                            num14 = vector2.y;
                        }
                        ...
                        Vector3 vector3 = (Vector3) ((vector2 - base.transform.position) / num13);
                        this.abnorma_jump_bite_horizon_v = new Vector3(vector3.x, 0f, vector3.z);
                        Vector3 velocity = base.rigidbody.velocity;
                        Vector3 vector5 = new Vector3(this.abnorma_jump_bite_horizon_v.x, num17, this.abnorma_jump_bite_horizon_v.z);
